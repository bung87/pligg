...
if(!isset($_POST['email_to_submit'])){ // we're not submitting the form
	...
	if(!$current_user->authenticated) {
		echo '<br/><p><div class="alert alert-block">'.$main_smarty->get_config_vars('PLIGG_Visual_Recommend_Logged_In').'</div></p>';
		die();
	}
	...
	if($time_since_last_email < Recommend_Time_Limit){
		echo '<br/><p><div class="alert alert-block">'.$main_smarty->get_config_vars('PLIGG_Visual_Recommend_Limit').'</div></p>';
		die();
	}
		...
		if($cansend >= 10){
		  ...
		  $mailer = new PliggMailer($subject, $body, Send_From_Email, $addresses);
			...
			if ($mailer->send()){
				...
				echo '<br/><p><div class="alert alert-block">'.$main_smarty->get_config_vars('PLIGG_PassEmail_SendFail').'</div></p>';
			}
		} else {
			echo '<br/><p><div class="alert alert-block">'.$main_smarty->get_config_vars('PLIGG_PassEmail_SendFail').'</div></p>';
		}
	} else {
		echo '<br/><div class="alert alert-block">'.$main_smarty->get_config_vars('PLIGG_PassEmail_SendFail').'</div>';
	}
    } else {
