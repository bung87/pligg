...
/* removed by bung
		 ...
		 *added by bung
		*/
			if(preg_match("@<meta name=[\"']?keywords[\"']? content=[\"']?([^<]*?)[\"']? />@", $this->html, $matches)) {
			 $tags=preg_split("/[\s,]+/", $matches[1]);
...
/////
	...
	function fill_smarty($smarty, $type='full'){
		...
		$title_short = htmlspecialchars(utf8_wordwrap($this->title, 40, "...", 1));
		...
		if(Enable_Tags){
			...
			if (!empty($this->tags)) {
				...
				$sep_count = substr_count($tags_words, ',');
				if ($sep_count > 1){$tags_words = $tags_words;}
...
function get_group_membered()
	{
		...
		if ($this->group_membered)
			foreach($this->group_membered as $results)
				$output .= "<li><a class='group_member_share' href='".my_base_url.my_pligg_base."/group_share.php?link_id=".$this->id."&group_id=".$results->group_id."&user_id=".$current_user->user_id."' >".$results->group_name."</a></li>";
       ...
       else
       {
   		...
   		if($iconv_loaded){
   			if(isset($headers['content-type']) && !empty($headers['content-type'])){
   				$charset='';
       		if (preg_match('/charset=(.+)$/',$headers['content-type'],$m)){
      		...
      		// exit($body);
      		if(preg_match("/<meta[^>]+?charset=[^\w]?([-\w]+)/i",$body,$temp)){
      			$charset=strtoupper($temp[1]);
      		}
      		
	        if($charset){
	        	//add IGNORE,otherwise will occur some wrong,if the in_charset is gb2312,by bung
	       	 $res = iconv($charset,"UTF-8//IGNORE",$body);
	       	 if(!$res){ return $body;}
	       	 return ($res);
	   		}
