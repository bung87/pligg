...
//add by bung
function group_print_avatar($requestID)
{
	global $db,$main_smarty,$the_template;
	if (!is_numeric($requestID)) die();
	...
	if($group)
	{
		...
		if($group_avatar == "uploaded" && file_exists(mnmpath."avatars/groups_uploaded/".$group_id."_".group_avatar_size_width.".jpg"))
			...
			$imgsrc = my_base_url . my_pligg_base."/templates/".$the_template."/images/group_small.gif";
		$main_smarty->assign('imgsrc', $imgsrc);	
		...
		//check logged or not
		// $main_smarty->assign('user_logged_in', $current_user->user_login);
		//removed by bung,this variable has been assigned
		$group_output .= $main_smarty->fetch(The_Template . '/group_avatar.tpl'); 
				
		$index++;
	}
	return $group_output;
}
function group_print_summary($requestID,$short=false)
{
	global $db,$main_smarty,$the_template;
	if (!is_numeric($requestID)) die();
	...
	if($group)
	{
		...
		$group_output .= $main_smarty->fetch(The_Template . ($short ? '/group_summary2.tpl':'/group_summary.tpl')); 
