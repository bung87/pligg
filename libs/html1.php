...
function do_pages($total, $page_size, $thepage, $fetch = false) {
	...
	if ($URLMethod == 1) {
		...
		$output .= '<div class="pagination-small pagination-centered"><ul class="pager">';

		if($current==1) {
			$output .= '<li class="previous disabled"><a href="#">&#171; '.$main_smarty->get_config_vars("PLIGG_Visual_Page_Previous"). '</a> </li>';
		} else {
			$i = $current-1;
			if ((pagename == "index" || pagename == "published")  && $i==1)
				$output .= '<li class="previous"><a href="'.($query ? '?' : './').$query.'">&#171; '.$main_smarty->get_config_vars("PLIGG_Visual_Page_Previous").'</a></li>';
			else
				$output .= '<li class="previous"><a href="?page='.$i.$query.'">&#171; '.$main_smarty->get_config_vars("PLIGG_Visual_Page_Previous").'</a></li>';
		}
		...
		if($current<$total_pages) {
			$i = $current+1;
			$output .= '<li class="next"><a href="?page='.$i.$query.'"> '.$main_smarty->get_config_vars("PLIGG_Visual_Page_Next"). ' &#187;' . '</a></li>';
		} else {
			$output .= '<li class="disabled next"> <a href="#">'.$main_smarty->get_config_vars("PLIGG_Visual_Page_Next"). ' &#187;' . '</a></li>';
		}
		$output .= "</ul></div>\n";
	}	
