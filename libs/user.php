...
//print_r($mail);					
	...
	function all_stats($from = false) {
		...
		$this->total_saved=	$db->get_var('SELECT count(*) FROM ' . table_saved_links . ' WHERE saved_user_id = "'.$this->id.'"');
	...
	function fill_smarty($main_smarty, $stats = 1){
		...
		if($stats == 1){		
			...
			$main_smarty->assign('user_total_saved', $this->total_saved);
