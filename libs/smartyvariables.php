...
/*add by bung*/
$main_smarty->assign('user_url_friends', getmyurl('user_friends', $current_user->user_login, 'viewfriends'));
$main_smarty->assign('user_url_friends2', getmyurl('user_friends', $current_user->user_login, 'viewfriends2'));
function get_group_membered()
	{
		global $db, $main_smarty, $rows,$current_user;
		$current_userid = $current_user->user_id;
		if (!isset($group_membered) && $current_userid)   
 $group_membered = $db->get_results("SELECT DISTINCT group_id,group_name FROM " . table_groups . " LEFT JOIN ".table_group_member." ON member_group_id=group_id AND member_user_id = $current_userid WHERE group_status = 'Enable' AND member_status='active'");
		$output = '';
		if ($group_membered)
			foreach($group_membered as $results)
				$output .= "<li><a class='group_member_share' href='".my_base_url.my_pligg_base."/group_share.php?link_id="."linkid"."&group_id=".$results->group_id."&user_id=".$current_user->user_id."' >".$results->group_name."</a></li>";

		return $output;

	}

$main_smarty->assign('get_group_membered', get_group_membered());
...
$main_smarty->assign('URL_Profile', getmyurl('profile')); //moveto smartyvar by bung
$main_smarty->assign('user_tabs',array('history','published','shaken','profile','commented' ,'voted','saved'));
?>
