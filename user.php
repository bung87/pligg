...
//	$page_header = $user->username;
	...
	$main_smarty->assign('user', $user->username);
...
//add by bung
	...
	$group = $db->get_results("SELECT distinct(group_id) as group_id FROM " . table_groups . " ORDER BY group_status DESC, ".$order_by." LIMIT 4 ");
	if ($group)
	{
		foreach($group as $groupid)
		{
			$group_display .= group_print_avatar($groupid->group_id);
		}
		$main_smarty->assign('sidebar_usergroups', $group_display);
	}	
