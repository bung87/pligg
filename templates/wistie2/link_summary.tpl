...
<div class="stories clearfix" id="xnews-{$link_shakebox_index}" {if $link_shakebox_currentuser_reports gt 0} style="opacity:0.3;filter:alpha(opacity = 30)"{/if}>
{else}
<div class="stories-wide clearfix" id="xnews-{$link_shakebox_index}" {if $link_shakebox_currentuser_reports gt 0} style="opacity:0.3;filter:alpha(opacity = 30)"{/if}>
{/if}
{checkActionsTpl location="tpl_pligg_story_start"}
	...
	<div class="headline clearfix">
		...
		{/if}			
			...
			{checkActionsTpl location="tpl_pligg_story_votebox_start"}
					...
					<a class="votenumber" id="xvotes-{$link_shakebox_index}" href="javascript:{$link_shakebox_javascript_vote}">{$link_shakebox_votes}</a>
			
				<br />
				<span class="xvote" id="xvote-{$link_shakebox_index}">				
				<a class="btn btn-mini{if $link_shakebox_currentuser_votes neq 0 || $user_logged_in eq $link_submitter} disabled{/if}" href="javascript:{$link_shakebox_javascript_vote}">
							<i class="icon-thumbs-up black"></i>{*#PLIGG_Visual_Vote_For_It#*}</a>			
				<a class="btn btn-mini {if $link_shakebox_currentuser_reports neq 0 || $user_logged_in eq $link_submitter} disabled{/if}" href="javascript:{$link_shakebox_javascript_report}">{*#PLIGG_Visual_Vote_Bury#*}<i class="icon-thumbs-down black"></i></a>	
				</span>
		...
		*}
		...
		{/if}	
			...
			<div class="pull-right">
				...
				<a class="avatar" id="ls_avatar-{$link_shakebox_index}" title="{$link_submitter}" href="{$submitter_profile_url}">
				 {if $UseAvatars neq "0"}<img src="{$Avatar_ImgSrc}" class="avatar"/>{/if} {*$link_submitter*}{*if $submitter_rank neq ''} (#{$submitter_rank}){/if*}<p>{$link_submitter}</p></a> 
	...
	<div class="storyfooter clearfix">
		...
		{if $user_logged_in}  |&nbsp;
			...
			{else}
				<a class="linksummaryRemoveLink" href="{$user_url_remove_links}" date-action="show_hide_user_links(document.getElementById('stories-{$link_shakebox_index}'));"><i class="black icon-remove"></i>{#PLIGG_MiscWords_Save_Links_Remove#}</a>
			{/if}
		...
		{if $link_shakebox_currentuser_votes eq 1 && $link_shakebox_currentuser_reports eq 1}|&nbsp;
			<span class="linksummaryUnvote"><a href="javascript:{$link_shakebox_javascript_unvote}">{#PLIGG_Visual_Unvote_For_It#}</a></span>&nbsp; 
		{/if*}
		...
		{if $enable_group eq "true" && $user_logged_in}
			|&nbsp;<a id = "group_share-{$link_shakebox_index}" href="javascript:;" class="group_sharing" title="" data-linkid="{$link_id}"><i class="black icon-share"></i>{#PLIGG_Visual_Group_Share#}</a>
