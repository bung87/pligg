  ...
  <div style="padding-left:20px;font-weight:bold;">
	...
	{$hidden_token_admin_users_create}
	  ...
	  <label for="username">{#PLIGG_Visual_Register_Username#}:</label>
		...
		{if isset($username_error)}{ foreach value=error from=$username_error }<br /><div class="alert alert-block">{$error}</div><br />{ /foreach }<br />{/if}	
		<div style="clear:both;"></div>
		<label for="email">{#PLIGG_Visual_Register_Email#}:</label>
		<div class="div_texbox">
	    <input name="email" type="text" class="textbox" id="city" value="" />
		</div>
		{if isset($email_error)}{ foreach value=error from=$email_error }<br /><div class="alert alert-block">{$error}</div><br />{ /foreach }<br />{/if}	
		<div style="clear:both;"></div>
		...
		{if isset($password_error)}{ foreach value=error from=$password_error }<br /><div class="alert alert-block">{$error}</div><br />{ /foreach }<br />{/if}	
		<div style="clear:both;"></div>
		<div class="buton_div">
