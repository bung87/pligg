...
{checkActionsTpl location="tpl_pligg_profile_info_start"}
		...
		<h2>{#PLIGG_Visual_Profile_UploadAvatar2#}:</h2>
			{if $user_avatar_source eq "useruploaded"}
				...
				<form method="POST" enctype="multipart/form-data" name="image_upload_form" action="{$form_action}">
					...
					<input type="submit" value="{#PLIGG_Visual_Profile_AvatarUpload#}" name="action" class="btn btn-primary"><br>
				</form> 
			{/if}	
	{/if}
	...
	{if $UseAvatars neq false}
			...
			<input type="radio" name="avatarsource" value="useruploaded" {if $user_avatar_source eq "useruploaded"}CHECKED{/if}> {#PLIGG_Visual_Profile_UploadAvatar#}<br/><br/>
			<input type=submit name="save_profile" value="{#PLIGG_Visual_Profile_Save#}" class="btn">					
	{/if}
