...
{section name=live_item loop=$live_items}
		<ul class="unstyled">
			<li><a href="{$URL_user, $live_items[live_item].link_username}">{$live_items[live_item].link_username}</a> 
				{#PLIGG_Visual_User_NewsSent#}
				<a href="{$live_items[live_item].link_url}">{$live_items[live_item].link_title}</a>
				({$live_items[live_item].link_votes}{#PLIGG_Visual_Breadcrumb_Vote#}) {$live_items[live_item].link_date}{#PLIGG_Visual_Comments_Date#}
			</li>
		</ul>
{/section}	
{*section name=live_item loop=$live_items}
